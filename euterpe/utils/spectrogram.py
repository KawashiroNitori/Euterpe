...
def convert_to_mono(input_file: str, output_file: str):
    command = 'sox {0} {1} remix 1,2'.format(input_file, output_file)
    run(command, shell=True, stdin=PIPE, stdout=PIPE)
    return output_file 


def audio_to_spect(input_file: str, output_file: str):
    command = 'sox {0} -n spectrogram -Y 300 -X 50 -m -r -o {1}'.format(input_file, output_file)
    run(command, shell=True, stdin=PIPE, stdout=PIPE)
    return output_file
...
def slice_spect(input_file: str, output_path: str):
    img_file_cleaned = path.basename(input_file).replace('.png', '')
